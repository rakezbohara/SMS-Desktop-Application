...
public class DBHelper {
     ...
     */
        ...
        // TODO code ROOTlication logic here
        public DBHelper(){
    ...
    System.out.println("VendorError: " + ex.getErrorCode());
      ...
      /*   public Boolean studentExist(String name){
            ...
            String sql = "SELECT * FROM ROOT.STUDENTS_INFO WHERE fname like '"+name+"'";
   ...
   //public void senddata(){
    ...
    ImageIcon getimage(String i) {
        ...
        try{
            ...
            String sql = "SELECT IMAGE FROM ROOT.STUDENTS_INFO WHERE SID="+id;
      ResultSet rs = stmt.executeQuery(sql);
      //STEP 5: Extract data from result set
      while(rs.next()){
    ...
    return(rs);
        ...
        String qry = "UPDATE ROOT.STUDENTS_FEE SET CLEARED='"+payment+"', BALANCE='"+balance+"', PAID='"+amount+"' WHERE SID="+id;
        try{
        PreparedStatement st=conn.prepareStatement(qry);
    ...
    void updatefeestaff(String selectedID, String payment, int bal, int amt) {
        ...
        String amount = String.valueOf(amt);
         int id = Integer.parseInt(selectedID);
         System.out.println(id);
        String qry = "UPDATE ROOT.STAFF_SALARY SET CLEARED='"+payment+"', BALANCE='"+balance+"', PAID='"+amount+"' WHERE SID="+id;
        try{
        PreparedStatement st=conn.prepareStatement(qry);
...
public void addIdToClass(String idvalue,String classnum){
   ...
   void sendToRest(){
       try{
             String qry = "select * from root.students_info";
             ResultSet rs = stmt.executeQuery(qry);
             while (rs.next()) {
                 ...
                 String name;
                    if(mname.equals("")){
                        ...
                        name = fname+"%20"+mname+"%20"+lname;
                    }
          
                 String url = "http://localhost:80/sms/php/try.php?sid="+sid+"&name="+name+"&grade="+grade+"&gender="+gender;
   ...
   void sendToRestMarks(){
       String tbl[] = {"CLASS_NURSERY","CLASS_LKG","CLASS_UKG","CLASS_1","CLASS_2","CLASS_3","CLASS_4","CLASS_5","CLASS_6","CLASS_7","CLASS_8","CLASS_9","CLASS_10"};
    
       try{
             String qry = "select * from root.allsubjects";
             ResultSet rs = stmt.executeQuery(qry);
             while (rs.next()) {
                int sid = rs.getInt("ID"); 
                String subname = rs.getString("SUBJECTNAME");
                String fullmarks = rs.getString("FULLMARKS");
                String passmarks = rs.getString("PASSMARKS");
                String url = "http://localhost:80/sms/php/allsubjects.php?sid="+sid+"&subname="+subname+"&fullmarks="+fullmarks+"&passmarks="+passmarks;
                SendToRest rest = new SendToRest(url);
       ...
       try{
             String qry = "select * from root."+tbl[j];
             ResultSet rs = stmt.executeQuery(qry);
             while (rs.next()) {
                int sid = rs.getInt("SID");                
                String url = "http://localhost:80/sms/php/class.php?sid="+sid+"&class="+tbl[j];
                SendToRest rest = new SendToRest(url);
              }
       ...
       try{
             String qry = "select * from root.marks";
             ResultSet rs = stmt.executeQuery(qry);
             while (rs.next()) {
                 int sid = rs.getInt("SID");
                 String firter=rs.getString("FIRTER");
                 String secter = rs.getString("SECTER");
                 String thiter = rs.getString("THITER");
          
                
                String url = "http://localhost:80/sms/php/marks.php?sid="+sid+"&firter="+firter+"&secter="+secter+"&thiter="+thiter;
                SendToRest rest = new SendToRest(url);
   ...
   // }
    ...
    void sendToRestReport() {
        
       String month[] = {"BAISHAK","JESTHA","ASHADH","SHRAWAN","BHADRA","ASHWIN","KARTIK","MANGSIR","POUSH","MAGH","FALGUN","CHAITRA"};
       for(int j=0;j<month.length;j++){
       try{
             String qry = "select * from root."+month[j];
             ResultSet rs = stmt.executeQuery(qry);
             while (rs.next()) {
                int sid = rs.getInt("SID");    
                String patten = rs.getString("ATTENDANCE");
                String classp = rs.getString("PERFORMANCE");
                String homework = rs.getString("HOMEWORK");
                String testp = rs.getString("TEST");
                String extrac = rs.getString("EXTRA");
                String payment = rs.getString("PAYMENT");
                int tatten = rs.getInt("ATTENDANCETOTAL");
                
                System.out.println(sid+"/n");
                System.out.println(patten+"/n");
                System.out.println(classp+"/n");
                System.out.println(homework+"/n");
                System.out.println(testp+"/n");
                System.out.println(extrac+"/n");
                System.out.println(payment+"/n");
                System.out.println(tatten+"/n");
                
                String url = "http://localhost:80/sms/php/report.php?sid="+sid+"&month="+month[j]+"&patten="+patten+"&tatten="+tatten+"&homework="+homework+"&extrac="+extrac+"&classp="+classp+"&testp="+testp+"&payment="+payment+"&review=NeedaGuidance";
